...
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args): 
    ...
    for idx, batch_data in enumerate(loader): 
        ...
        for j in range(len(case_name)): 
            ...
            age=int(case_name[j].split('_')[-2].split('Y')[0]) 
        ...
        word_embeddings_lst = torch.stack(word_embedding_list, dim=0) 
        word_embeddings_lst = word_embeddings_lst.cuda(args.rank) 

        for param in model.parameters(): 
            param.grad = None 
        with autocast(enabled=args.amp):  
            logits = model(data, word_embeddings_lst) 
...
def val_epoch(model, loader, epoch, acc_func, args, model_inferer=None, post_sigmoid=None, post_pred=None):  
    ...
    with torch.no_grad(): 
        for idx, batch_data in enumerate(loader): 
            ...
            with autocast(enabled=args.amp): 
                logits = model_inferer(data, word_embeddings_lst) 
            ...
            if args.rank == 0: 
                ...
                Dice_CSF = run_acc.avg[1] 
                Dice_GM = run_acc.avg[2] 
                Dice_WM = run_acc.avg[3] 
                print(
...
def save_checkpoint(model, epoch, args, optimizer, scheduler, filename="model.pt", best_acc=0):  
            ...
            if args.rank == 0: 
                    ...
                    if semantic_classes is not None: 
                        for val_channel_ind in range(len(semantic_classes)): 
                            if val_channel_ind < val_acc.size: 
                                writer.add_scalar(semantic_classes[val_channel_ind], val_acc[val_channel_ind+1], epoch)
                ...
                if val_avg_acc > val_acc_max: 
                    ...
                    if args.rank == 0 and args.logdir is not None and args.save_checkpoint:  
                        save_checkpoint(
                            model, epoch, args, optimizer=optimizer, scheduler=scheduler, best_acc=val_acc_max
                        ) 
            if args.rank == 0 and args.logdir is not None and args.save_checkpoint:  
                save_checkpoint(model, epoch, args,optimizer=optimizer, scheduler=scheduler, best_acc=val_acc_max, filename="model_final.pt")  
                if b_new_best:  
