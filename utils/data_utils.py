...
def datafold_read(datalist, fold=1, key="training"): 
    ...
    for d in json_data: 
        for k, v in d.items(): 
            if isinstance(d[k], list): 
                d[k] = [iv for iv in d[k]] 
            elif isinstance(d[k], str): 
                d[k] = d[k] if len(d[k]) > 0 else d[k]
...
def get_loader(args):  
    ...
    train_files, validation_files = datafold_read(datalist=args.json_list, fold=args.fold)
